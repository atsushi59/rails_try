...
RSpec.describe SomeController, type: :controller do
  ...
  describe "GET #index" do
    ...
    it "extracts queries from session and processes them" do
      session[:query] = queries
      get :index
      # セッションからクエリを取得し、それを分割して配列に変換することを期待する
  ...
  describe "#calculate_travel_time" do
    ...
    context "公共交通機関を選択した場合" do
      ...
      it "公共交通機関での旅行時間が正しく計算されること" do
        expect(controller).to receive(:calculate_travel_time_by_public_transport).with(origin,
                                                                                       destination).and_return(60)
        controller.calculate_travel_time(place_detail)
